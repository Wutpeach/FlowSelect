...
- The old Tauri source, scripts, dependencies, and release wiring were removed only after the Electron build, packaging, sidecar, and acceptance checks were green on this task branch.
- `desktop-assets/` is now the only repo-visible source of truth for desktop icons, bundled binaries, and Pinterest sidecar lock metadata.
...
| Download flows | `src/electron-runtime/`, `electron/main.mts`, `desktop-assets/binaries/` | App build, runtime path tests, Pinterest sidecar build, and binary smoke checks passed after moving assets out of `src-tauri/`. | Passed |
