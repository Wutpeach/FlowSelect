...
Use `ace-tool` semantic search as the default way to locate code before editing.

- Use `mcp__ace-tool__search_context` for behavior lookup, ownership discovery, and cross-file pattern analysis.
- Use `rg`/`grep` only for exact literal matches or as a temporary fallback when `ace-tool` is unavailable.
- In fallback mode, document that you used literal search and verify results carefully before editing.
...
2. **Search with ace-tool First** - [!] Use `mcp__ace-tool__search_context` as default for code search
...
### Step 2.5: Run Code Search with ace-tool [!] REQUIRED

Before selecting files to edit, run `mcp__ace-tool__search_context` with a natural-language query to identify:
- the ownership files for the feature/bug
- nearby patterns to follow
- likely call sites and tests

Use `rg` only for exact string/value lookups after `ace-tool` narrows the area.
...
2. Write code according to guidelines
   ...
   --> Use `mcp__ace-tool__search_context` before editing to locate relevant code
   --> Use `rg`/`grep` only for exact literal lookup or fallback
...
2. **During development**:
   ...
   - [!] Default to `mcp__ace-tool__search_context` for code search
   - Use `rg`/`grep` only for exact literal matches or fallback
...
# Code search (default)
mcp__ace-tool__search_context(project_root_path="D:/FlowSelect", query="<natural language task query>")

# Exact literal search (only when needed)
rg "literal_to_find" .
